...
public class RevisionService {
    ...
    public List<RevisionDto> obtenerHistorial(String emailUsuario) {
        ...
        formularioRepo.findByUsuario_Id(idUsuario).forEach(f -> {
        ...
        requisitoRepo.findByUsuario_Id(idUsuario).forEach(r -> {
            historial.add(RevisionDto.builder()
                    ...
                    .subtitulo(r.getResena() != null && r.getResena().length() > 50
                            ...
                            : r.getResena())
                    .fecha(LocalDate.now()) // Requisito no tiene fecha_creacion, usar campo audit si existe o fecha
                                            // actual
                    .estado(r.getResultado() != null ? r.getResultado().getResultado() : "Pendiente")
                    .build());
        });
